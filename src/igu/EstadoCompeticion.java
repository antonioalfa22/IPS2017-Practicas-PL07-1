	...
	private boolean meterDatos() {
		...
		try {
			...
			for (Carrera carr : usuario.getCarreras()) {
				if(carr.getId() == c.getId()) flag = true;
			}
			if(flag) {
				Corredor corredor = null;
				for(Corredor corr: gestor.corredores) if(corr.getDni().equals(usuario.getDni())) corredor = corr;
				System.out.println(corredor);
				//VentanaPrincipal.gestorCarreras.deleteCorredor(usuario.getDni());
			}else {
				//VentanaPrincipal.gestorCarreras.deletePreinscrito(usuario.getDni());
			}
