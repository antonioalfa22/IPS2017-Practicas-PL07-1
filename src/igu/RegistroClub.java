...
public class RegistroClub extends JDialog {
	...
	private void registrar() {
		if(!(users == null) && !txtNombre.getText().equals("")) {
			...
			try {
				...
				gestor.getTodosLosCorredores(carrera).stream().forEach(x -> {
					if(listaUsuarios.stream().map(y -> y.getDni()).collect(Collectors.toList()).contains(x.getDni()))
						listaUsuarios.removeIf(z -> z.getDni().equals(x.getDni()));
				});
				listaUsuarios.forEach(x -> VentanaPrincipal.gestorCarreras.addPreeinscrito(x, carrera, fecha_insc));
